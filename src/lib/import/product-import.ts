...
const FIELD_ALIASES: Record<string, string[]> = {
  ...
  name: ["urun adi", "ad", "urun", "urun ismi", "isim", "name"],
...
/**
 * Excel/CSV buffer'ından toplu ürün+stok içe aktarma çekirdeği.
 * dryRun=true → hiçbir kayıt yapmadan rapor döner.
 * SKU ile eşleştirir: eşleşen güncellenir, eşleşmeyen yeni ürün oluşturulur.
 */
export async function importProductsFromBuffer(
  buf: Buffer,
  opts: { dryRun: boolean; userId: string; userName?: string | null },
): Promise<ImportReport> {
  const { dryRun, userId, userName } = opts;
  ...
  if (!dryRun && (created > 0 || updated > 0)) {
    await db.auditLog.create({
      data: {
        ...
        entityType: "Product",
        entityId: "bulk",
        newValue: { created, updated, skipped, totalRows: aoa.length - 1 } as Prisma.InputJsonValue,
      },
    });
  }
